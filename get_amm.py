...
with open('usages_des_produits_autorises_v3_utf8.csv', 'r+') as all_products, open('fichier_bio', 'w+') as bio :
    for i in all_products :
        if 'Utilisable en agriculture biologique' in i and 'Vigne' in i and ('Retrait' not in i) and 'Thrips' not in i and "Black rot" not in i and "Bactérioses" not in i and 'Excoriose' not in i and 'Erinose' not in i and 'Cochenilles' not in i and 'Aleurodes' not in i and 'Pourriture grise' not in i and 'Mouches' not in i and 'Stad. Hivern. Ravageurs' not in i and 'lack dead arm' not in i and 'Esca' not in i and 'Chenilles phytophages' not in i and 'Eutypiose' not in i and 'Acariens' not in i :
...
with open ('fichier_bio', 'r+') as lecture, open ('fichiers_intrants', 'w+') as intrants :
    already = []
    intrants.write('CUIVRE;;;;;;;;;SOUFRE;;;;;;;;;INSECTICIDE;;;;;;;;;CONFUSION;;;;;BIOCONTROLE;;;;;;;;\n')
    intrants.write('Spécialité Commerciale;Matière Active (M.A.);Autre;Concentration en M.A. (%);Dose homologuée (Kg ou L / ha);Kg / ha de M.A.;Nombre de traitements max;;;Spécialité Commerciale;Matière Active (M.A.);Autre;Concentration en M.A. (%);Dose homologuée (Kg ou L / ha);Kg / ha de M.A.;Biocontrôle (1/0);Nombre de traitements max;;Spécialité Commerciale;Matière Active (M.A.);Autre;Concentration en M.A. (%);Dose homologuée (Kg ou L / ha);Kg / ha de M.A.;Biocontrôle (1/0);Nombre de traitements max;;Spécialité commerciale;Matière Active (M.A);Biocontrôle(1/0);;;Spécialité Commerciale;Matière Active (M.A.);Autre;Concentration en M.A. (%);Dose homologuée (Kg ou L / ha);Kg / ha de M.A.;Biocontrôle (1/0);Insecticide;Nombre de traitements max;\n')
    for i in lecture :
        Autres = ''
        ...
        if "cuivre" in liste_lecture[8] :
            try :
                ...
                # !!! EMPTY space before g --> not to get kg (kilos) per L 
                    ...
                    Concentration = int(Concentration_nombre) / 100
            except : Concentration = 0
            #Dose calculation
            Dose = (Concentration / 100 ) * float(liste_lecture[17])
            Dose = round(Dose, 1)
            
            #Write into the file
            intrants.write("{0};{1};{2};{3};{4};{5};{6};;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;\n".format(liste_lecture[2], MA[0], Autres, Concentration, liste_lecture[17], Dose, liste_lecture[21]))
            liste_autres = liste_lecture[3].split(" | ")
            for i in liste_autres :
                if i == "" : continue
                intrants.write("{0};{1};{2};{3};{4};{5};{6};;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;\n".format(i, MA[0], Autres, Concentration, liste_lecture[17], Dose, liste_lecture[21]))
        ...
        # Extract sulphur information :
            ...
            #Write into the file
            intrants.write(";;;;;;;;;{0};{1};{2};{3};{4};{5};{6};{7};;;;;;;;;;;;;;;;;;;;;;;;\n".format(liste_lecture[2], MA[0], Autres, Concentration, liste_lecture[17], Dose, biocontrole, liste_lecture[21]))
            liste_autres = liste_lecture[3].split(" | ")
            for i in liste_autres :
                if i == "" : continue
                intrants.write(";;;;;;;;;{0};{1};{2};{3};{4};{5};{6};{7};;;;;;;;;;;;;;;;;;;;;;;;\n".format(i, MA[0], Autres, Concentration, liste_lecture[17], Dose, biocontrole, liste_lecture[21]))
        
        #Extract insecticide information
        elif "Insecticide" in liste_lecture[9] and ("Spinosad" in liste_lecture[8] or "Bacillus" in liste_lecture[8] or "pyréthrines" in liste_lecture[8]): 
            try :
                Concentration_liste = MA[1].split(') ')
                Concentration_brute = Concentration_liste[1]
                Concentration_nombre = ""
        ...
        #Extract pheromones information
            ...
            MA = liste_lecture[8].replace("(Straight Chain Lepidopteran Pheromones)", "").replace("|", "+")
            #Get if the product is classified as "biocontrole" (yes/no)
            if "biocontrôle" in liste_lecture[7] :
                biocontrole = 1
            else :
            ...
            #Write into the file
            intrants.write(";;;;;;;;;;;;;;;;;;;;;;;;;;;{0};{1};{2};;;;;;;;;;;\n".format(liste_lecture[2], MA, biocontrole))
            liste_autres = liste_lecture[3].split(" | ")
            for i in liste_autres :
                if i == "" : continue
                intrants.write(";;;;;;;;;;;;;;;;;;;;;;;;;;;{0};{1};{2};;;;;;;;;;;\n".format(i, MA, biocontrole))
            ...
            #write into the file
            intrants.write(";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;{0};{1};{2};{3};{4};{5};{6};{7};{8}\n".format(liste_lecture[2], MA[0], Autres, Concentration, liste_lecture[17], Dose, biocontrole, insecticide, liste_lecture[21]))
            liste_autres = liste_lecture[3].split(" | ")
            for i in liste_autres :
                if i == "" : continue
                intrants.write(";;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;{0};{1};{2};{3};{4};{5};{6};{7};{8}\n".format(i, MA[0], Autres, Concentration, liste_lecture[17], Dose, biocontrole, insecticide, liste_lecture[21])) 
    intrants.close()
    lecture.close()
...
with open ('fichiers_intrants', 'r+') as intrants :
    for intrant in intrants:
        ...
        if intrant[:6]=="CUIVRE": continue
        if intrant[:9]=='Spécialit' : continue
...
with open('intrants_final.csv', 'w+') as intrants :
    for i in range(0, 500) :
        intrants.write(f'{list_cuivre[i]};{list_soufre[i]};{list_insecticide[i]};{list_confusion[i]};{list_biocontrole[i]}\n')
intrants.close()
